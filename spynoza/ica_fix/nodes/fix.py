...
COPIED FROM WOLGANG PAULI's NIPYPE FORK (NOT YET MERGED INTO NIPYPE MASTER).
https://github.com/wmpauli
...
class FeatureExtractorInputSpec(CommandLineInputSpec):
    mel_ica = Directory(exists=True, copyfile=False,
...
class AccuracyTester(CommandLine):
    ...
    def _list_outputs(self):
        ...
        if isdefined(self.inputs.output_directory):
            outputs['output_directory'] = Directory(exists=False,
                                                    value=self.inputs.output_directory)
        else:
            outputs['output_directory'] = Directory(exists=False,
                                                    value='accuracy_test')
        return outputs
...
class ClassifierInputSpec(CommandLineInputSpec):
    ...
    thresh = traits.Int(argstr='%d', desc='Threshold for cleanup.', position=-1,
                        mandatory=True)
...
class CleanerInputSpec(CommandLineInputSpec):
    artifacts_list_file = File(exists=True, argstr='%s', position=1,
                               mandatory=True,
                               desc='Text file listing which ICs are artifacts; can be the output from classification or can be created manually')
    ...
    highpass = traits.Float(argstr='-m -h %f',
                            desc='cleanup motion confounds', value=100,
                            position=2)
