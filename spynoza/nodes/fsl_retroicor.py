...
class PreparePNMInput(FSLCommandInputSpec):
    ...
    sampling_rate = traits.Int(100, argstr='-s %d', desc='sampling_rate')
    ...
    tr = traits.Float(3.330, argstr='--tr=%f', usedefault=True)    
...
class PreparePNM(FSLCommand):
    ...
    def _list_outputs(self):
        ...
        for key in ['time', 'resp', 'card']:        
            outputs[key] = os.path.join(os.getcwd(),
                                        self.inputs.prefix + '_%s.txt' % key)
